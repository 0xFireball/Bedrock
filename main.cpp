...
static bool isBlockSolid(int x, int y, int z, util::vec3f face) {
    ...
    if (selectSolid)
        ...
        return Block::block(block)->isSolid || block == TALL_GRASS_ID || block == FLOWER_GRASS_ID ||
               block == MOUNTAIN_GRASS_ID;
}
...
/*static*/ void doSelect() {
    ...
    util::vec3f toUnit = util::vec3f{curUnit->x, curUnit->y, curUnit->z} -
                         (util::vec3f{blockSelection.x, blockSelection.y, blockSelection.z} + 0.5f);
    ...
    for (Entity *e : world.entities) {
        using std::max;
        if ((util::vec3f{curUnit->x, curUnit->y, curUnit->z} - util::vec3f{e->x, e->y, e->z}).length() +
            (max(max(e->rx, e->ry), e->rz) * 1.73205080757) > curUnit->viewDistance)
            continue;
...
#define max(a, b) (a>b?a:b)
    vec3i cMini = {min(blockSelection.x, corner1->x), min(blockSelection.y, corner1->y),
                   min(blockSelection.z, corner1->z)};
    vec3i cMaxi = {max(blockSelection.x, corner1->x), max(blockSelection.y, corner1->y),
                   max(blockSelection.z, corner1->z)};
#undef min
#undef max
...
#define max(a, b) static_cast<float>(a>b?a:b)
    vec3f cMin = vec3f{min(blockSelection.x, corner1->x), min(blockSelection.y, corner1->y),
                       min(blockSelection.z, corner1->z)} - SELECT_FACE_OFFSET;
    vec3f cMax = vec3f{max(blockSelection.x, corner1->x), max(blockSelection.y, corner1->y),
                       max(blockSelection.z, corner1->z)} + SELECT_FACE_OFFSET;
#undef min
#undef max
    vec3f dim = (cMax - cMin) + 1;
        ...
        case CommandEnum::MOVE:
            ...
            if (input::mouseLeftClicked && isBlockSelected && !inHUD) {
                ...
                curUnit->cmdQueue.push(
                        new MoveCommand({static_cast<float>(blockSelection.x) + 0.5f,
                                         static_cast<float>(blockSelection.y) + 0.5f,
                                         static_cast<float>(blockSelection.z) + 0.5f})
                );
                selecting = 0;
...
#undef max
    ...
    std::string selection = isBlockSelected ? Block::block(
            world.getBlock(blockSelection.x, blockSelection.y, blockSelection.z))->name :
...
void reshape(int w, int h) {
    ...
    perspectiveGL(FoVy, (GLfloat) width / (GLfloat) height, ZNEAR, ZFAR);
